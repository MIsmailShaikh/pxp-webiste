// Initialize Lucide Icons
    ...
    if (window.lucide) window.lucide.createIcons();
        ...
        if (textElement) {
            ...
            words = document.querySelectorAll('.reveal-word');
        }
        ...
        // FOLDER ANIMATION TRIGGER
        const folderContainer = document.querySelector('.folder-stack');
        const folderTrigger = document.getElementById('folder-trigger');

        const folderObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    if (folderContainer) folderContainer.classList.add('active');
                } else {
                    if (folderContainer) folderContainer.classList.remove('active');
                }
            });
        }, { threshold: 0.5 });

        if (folderTrigger) {
            folderObserver.observe(folderTrigger);
        }
